...
%     for E=1:length(EWfiles)
 for E=[ 1 2 3 4 5 6 8 9] 
                ...
                if ~isempty(maxR) & ~isempty(minR)
                    ...
                    if length(peaks)>3
                        ...
                        for p=1:length(peaks)
                            ...
                            [dps,IX]=sort(dp);
                            [pp,pre_p_idx]=max(dps(dps<0)); % find closest small peak
                            
                            if ~isempty(pp)
                                
                                pre_peak(p)=smallpeaks(IX(pre_p_idx),1);
                                FN=abs(pre_peak(p)-peaks(p,1));
                                p_hC(p)=(C(peaks(p,1))-C(pre_peak(p)))/FN; % get O2 concentration for peak and pre-peak and subtract
    ...
    if binningtype=='RBW'
        ...
        xlabel('peak RBW')
        ylabel('Head dC (%O2/s)')
        title (['dC/T_vs_RBW_head pooled  ' num2str(lo) '-' num2str(hi) ' deg'])
        ylim([-0.009 0.009])
    else
        ...
        ylabel('peak RBW')
        xlabel('Head dC (%O2/s)')
        title (['C/T_vs_RBW_head pooled  ' num2str(lo) '-' num2str(hi) ' deg ..'  name])
    end
    %%%%
...
saveas(gcf,['dCdT_vs_RBW ' num2str(lo) '-' num2str(hi) 'pooled.fig'])
disp('done')
