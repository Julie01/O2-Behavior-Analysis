...
%     for E=[ 1 2 3 4 5 6 8 9] 
                ...
                if ~isempty(maxR) & ~isempty(minR)
                    ...
                    if length(peaks)>3
                        ...
                        for p=1:length(peaks) %for each peak:
                            dp = (smallpeaks(:,1)-peaks(p,1)); % (1)distances between peak and all small peaks
                            [dps,IX]=sort(dp);%sort them
                            [pp,pre_p_idx]=max(dps(dps<0)); %find the index of the smallest negative distance: pre_p_idx
                            
                            if ~isempty(pp)
                                pre_peak(p)=smallpeaks(IX(pre_p_idx),1); % put into pre_peak vector
                                 FN=abs(pre_peak(p)-peaks(p,1));
                                p_hC(p)=(C(peaks(p,1))-C(pre_peak(p)))/FN;% find O2 concentrations at these timepoints
                            else
                                pre_peak(p)=NaN;
                                p_hC(p)=NaN;
                            end
    ...
    if binningtype=='RBW'
        ...
        xlabel('peak RBW')
        ylabel('Head dC/T (%O2/s)')
        title (['dCdT_vs_RBW_head  ' num2str(lo) '-' num2str(hi) ' deg ..'  name])
        ylim([-0.009 0.009])
        ...
        ylabel('peak RBW')
        xlabel('Head dC (%O2/s)')
        title (['CdT_vs_RBW_head  ' num2str(lo) '-' num2str(hi) ' deg ..'  name])
...
if sumAll==1
    saveas(gcf,['dCdT_vs_RBW_sum ' num2str(lo) '-' num2str(hi) '.fig'])
    disp('sum')
else
    saveas(gcf,['dCdT_vs_RBW_head ' num2str(lo) '-' num2str(hi) '.fig'])
end
disp('done')
