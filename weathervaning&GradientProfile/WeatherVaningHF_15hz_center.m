   ...
   disp('...loading Tracks')
    ...
    for T= 1:length(Tracks)
        ...
        if  Tracks(1,T).Frames(end)>3000 && Tracks(1,T).Frames(1)>1000
            ...
            RunFrames1={NaN};
            for ii=1:length(RunStart)
                RunX1{ii}=RunX(RunStart(ii):5:RunEnd(ii)); %downsample
                RunY1{ii}=RunY(RunStart(ii):5:RunEnd(ii));
                RunFrames1{T,ii}=Tracks(1,T).Frames((RunStart(ii):5:RunEnd(ii)));
            end
